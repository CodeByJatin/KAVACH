...
from database import db, User, Domain, Asset, IPRecord, ChatHistory
...
def get_rag_context(user_id, asset_id=None, user_message=''):
    """Assembles security context from files and database.
    Only loads heavy knowledge-base context when the question is security-related
    or targets a specific asset. Casual conversation gets no RAG injection.
    """
    context = ""
    
    # Detect if the message is security / technical in nature
    security_keywords = [
        'tls', 'ssl', 'cipher', 'pqc', 'quantum', 'nist', 'vulnerability', 'risk',
        'scan', 'asset', 'certificate', 'key', 'encrypt', 'attack', 'threat',
        'remediat', 'patch', 'firewall', 'malware', 'phishing', 'exploit',
        'compliance', 'audit', 'posture', 'cbom', 'score', 'rating',
        'kyber', 'ml-kem', 'dilithium', 'domain', 'subdomain', 'server',
        'nginx', 'apache', 'config', 'security', 'crypto', 'hack',
        'mitm', 'dnssec', 'hsts', 'cve', 'owasp', 'pentest', 'siem',
        'protect', 'defend', 'secure', 'infrastructure', 'rsa', 'ecdsa',
        'fix', 'upgrade', 'migrate', 'recommend', 'help me', 'how to',
        'what is', 'explain', 'kavach'
    ]
    msg_lower = user_message.lower()
    is_security_query = any(kw in msg_lower for kw in security_keywords)
    
    # 1. Load Standards & Glossary ONLY for security-related questions or asset-specific queries
    if is_security_query or asset_id:
...
def api_chat():
    ...
    rag_context = get_rag_context(session['user_id'], asset_id, user_message)
    
    # Build a smart system prompt
    system_prompt = (
        "You are the KAVACH AI Assistant — a smart, friendly, and professional cybersecurity co-pilot.\n"
        "You specialize in Post-Quantum Cryptography (PQC), TLS analysis, and infrastructure security, "
        "but you are also a well-rounded conversational AI that can chat naturally.\n\n"
        "BEHAVIORAL RULES:\n"
        "- If the user greets you casually (hello, hi, hey, etc.), respond warmly and briefly. Ask how you can help.\n"
        "- If the user asks a general knowledge question, answer it concisely and accurately.\n"
        "- If the user asks about cybersecurity, scanning, PQC, NIST, TLS, or their infrastructure, "
        "provide expert-level advice using the CONTEXT below.\n"
        "- Keep responses concise: prefer bullet points and short paragraphs.\n"
        "- Use markdown formatting when helpful (bold, code blocks, lists).\n"
        "- Do NOT volunteer NIST standards or technical jargon unless the user specifically asks about security topics.\n"
        "- Be natural. Match the user's tone — casual for casual, technical for technical.\n"
    )
    
    if rag_context:
        system_prompt += f"\nSECURITY CONTEXT (use only when relevant to the question):\n{rag_context}"
    ...
    def generate():
        ...
        try:
            ...
            err_msg = f"Error connecting to AI: {str(e)}"
            full_response = err_msg
            yield err_msg
            
        if full_response and user_message:
            with app.app_context():
                msg_user = ChatHistory(user_id=user_id, role='user', content=user_message)
                db.session.add(msg_user)
                msg_ai = ChatHistory(user_id=user_id, role='assistant', content=full_response)
...
def api_report(domain_id=None):
    ...
    if domain_id:
        ...
        assets_query = Asset.query.filter(Asset.domain_id == domain_id, Asset.pqc_score.isnot(None)).all()
        report_title = f"KAVACH Security Report: {domain.domain_name}"
        download_name = f"kavach_report_{domain.domain_name}.pdf"
    else:
        assets_query = Asset.query.join(Domain).filter(Domain.user_id == session['user_id'], Asset.pqc_score.isnot(None)).all()
    ...
    # Asset List Table
    headers = ['#', 'Subdomain', 'IP Address', 'Open Ports', 'TLS Version', 'Cipher Suite', 'Risk Level', 'PQC Score']
    data = [headers]
    ...
    for index, a in enumerate(assets):
        # Format the IP addresses to wrap and not overflow
        raw_ip = a.get('ip_address') or 'N/A'
        ip_list = [ip.strip() for ip in raw_ip.split(',')]
        if len(ip_list) > 2:
            ip_str = '<br/>'.join(ip_list[:2]) + '<br/>...'
        else:
            ip_str = '<br/>'.join(ip_list)
            
        # Format TLS to stack vertically
        raw_tls = a.get('tls_version') or 'N/A'
        tls_str = raw_tls.replace(', ', '<br/>')

        # Render cipher without any forced truncation so it wraps natively
        raw_cipher = a.get('cipher_suite') or 'N/A'
        
        # Render ports wrapping via commas
        raw_ports = a.get('open_ports') or 'None'
        ports_str = raw_ports.replace(', ', '<br/>')

        row = [
            str(index + 1),
            Paragraph(a.get('subdomain') or 'N/A', cell_style),
            Paragraph(ip_str, cell_style),
            Paragraph(ports_str, cell_style),
            Paragraph(tls_str, cell_style),
            Paragraph(raw_cipher, cell_style),
    ...
    col_widths = [20, 160, 90, 60, 70, 170, 80, 80]
    ...
    ts = TableStyle([
        ...
        ('ALIGN', (6, 0), (7, -1), 'CENTER'), # Center align Risk and Score
        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, 0), 10),
        ('BOTTOMPADDING', (0, 0), (-1, 0), 8),
