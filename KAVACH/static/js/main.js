...
function appendMessage(content, role, id = null) {
    ...
    msgDiv.innerHTML = `
        <div class="${innerClass} p-3 max-w-[85%] shadow-sm animate-fade-in">
            <p class="text-xs leading-relaxed" ${id ? `id="${id}"` : ''}>${content}</p>
        </div>
    `;
...
window.askAi = function(predefinedMessage) {
    const panel = document.getElementById('chatPanel');
...
function renderPostureTable(data) {
    ...
    tbody.innerHTML = html || '<tr><td colspan="6" class="px-6 py-8 text-center text-slate-500">No assets found matching the filter.</td></tr>';
}
...
async function initDiscovery() {
    ...
    if(!discoveryPollingInterval) { discoveryPollingInterval = setInterval(refreshDomainSidebar, 3000); }
}
...
async function refreshDomainSidebar() {
    try {
        ...
        if(!currentDiscoveryDomainId && domains.length > 0) { 
            ...
            // Re-fetch assets if active process is running
            if (current && (current.discovery_status === 'discovering' || current.status === 'scanning')) {
                await fetchDiscoveryAssets(currentDiscoveryDomainId);
            }
        }
    } catch(e) { console.error(e); }
}
...
function renderDiscoveryTableHeader() {
    ...
    if (currentDiscoveryTab === 'ips') {
        html += `<th class="px-6 py-4">IP Address</th><th class="px-6 py-4">Location</th>`;
        ...
        html += `<th class="px-6 py-4">IP Address</th>`;
    }
